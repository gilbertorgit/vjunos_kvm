...
class ConsoleConfig:
    ...
    def junos_vsrx3_specific_config(self, hostname):
        print("-" * 40, f"Configuring vSRX3 specific config: {hostname}")
        child = self.get_console(hostname)
        self.logfile_pexpect_start(child, hostname)
        child.send("\r\r\r")
        # Step 8: Delete Configs
        ...
        child.expect("root.*", timeout=60)
        self.logger.debug("Deleting chassis, system and protocols")
        child.sendline("delete security")
        child.sendline("delete system autoinstallation")

        # Step 9: FPC LITE-MODE
        print("Step 5: INET6/MPLS/ISO - PACKET-BASED")
        self.logger.debug("Configuring packet-based")
        child.sendline("set security forwarding-options family inet6 mode packet-based")
        child.sendline("set security forwarding-options family mpls mode packet-based")
        child.sendline("set security forwarding-options family iso mode packet-based")
        child.send("\r")
        child.expect("root.*", timeout=60)
        child.sendcontrol("]")

    def junos_vex_specific_config(self, hostname):
        print("-" * 40, f"Configuring vjunos-switch specific config: {hostname}")
        child = self.get_console(hostname)
        self.logfile_pexpect_start(child, hostname)
        child.send("\r\r\r")
        # Step 8: Delete Configs
        ...
        child.sendline("delete system processes dhcp-service")
        child.sendline("delete protocols router-advertisement")
        child.sendline("set system commit synchronize")
    ...
    def vsrx3_config(self, hostname, mgmt_ip):
        self.logger.debug("Starting vsrx3_config")
        if self.get_console_prompt(hostname):
            self.logger.debug("received TRUE from get_console_prompt")
            self.junos_common_config(hostname, mgmt_ip)
            self.junos_vsrx3_specific_config(hostname)
            self.junos_commit_quit(hostname)

            print("-" * 20, f' {hostname} configuration completed\n')

    def vex_config(self, hostname, mgmt_ip):
        self.logger.debug("Starting vex_config")
        if self.get_console_prompt(hostname):
            self.logger.debug("received TRUE from get_console_prompt")
            self.junos_common_config(hostname, mgmt_ip)
            self.junos_vex_specific_config(hostname)
            self.junos_commit_quit(hostname)
